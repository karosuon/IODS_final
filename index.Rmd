...
#Abstract
Religious identity is a complicated question which can be measured different ways. In this course work, the religiousness of European working age population is studied by using the dataset by *European Social Survey (ESS)*. The main question is to examine how religious beliefs and customs predict belonging to some religion or denomination. The impact of demographic variables (age and gender) and participants' home countries are studied also. Logistic regression is used in the analyses. Regarding the variables describing religious customs and beliefs, attending to religious services and having religious image of oneself seem to predict belonging to religion more than praying. European population is not homogeneous	with what it comes to religion. Gender differences of belonging to religion are quite small in this dataset, but age has a clear, positive association with belonging to religion.

#Research questions
I study how religious beliefs and customs predict belonging to some religion or denomination. I have the following hypotheses:
...
* 4. Countries differ with their religiousness: I expect Sweden, the Netherlands, and some of the Easter European countries (e.g., Estonia and Czech Republic ) to have smaller odds of belonging to religion than Finnish participants, and catholic countries, for instance, Poland and Spain, to have greater odds of belonging to religion.
...
Next I examine the earlier hypothesis with logistic regression. Logistic regression, also called a logit model, is used to model dichotomous outcome variables (in this study, belonging to religion or denomination). In the logit model the log odds of the outcome is modeled as a linear combination of the predictor variables. Independent variables are the question how religious participant sees oneself, how often participant prays, how often participant attends to religious services, and the country, gender and age of the participants. Concerning the country, Finland is the reference group, and concerning the gender, women are the reference group.

Below we have the results of logistic regression. We see that all of the countries differ statistically significantly from Finland. All of the variables describing religious customs and how religious the partipant sees oneself, are also associated with belonging to a religious group. Surprisingly, being male has weak but positive association with belonging to religion. As expected, age has positive association with belonging to religion.
...
Below we have the odds ratios and their confidence intervals (95 %). Odd ratio does not mean the same as probability: Probability ranges from 0 and 1,  ddds range from 0 and positive infinity. The odds increase as the probability increases or vice versa.

As expected, holding other variables at a fixed value, having a religious image of self is associated positively with belonging to religion (OR=2.6). Similarly, frequency of praying (OR=1.5) and frequency of attending to religious services (OR=2.4) are associated positively with belonging to religion. Yet, praying seems less important part of belonging to religion than the other two variables.

There are huge differences between the countries: As expected, participants from Sweden, the Netherlands, and Estonia and Czech Republic to have smaller odds of belonging to religion than Finnish participants, and catholic countries, for instance, Poland and Spain, to have greater odds of belonging to religion. The Israeli participants have the greatest odds (OR=382.5). Other than that, the countries with the greatest odds are Lithuania (OR=5.5) and Poland (OR=4.5), and on the other extreme there are Czech Republic (OR=0.3) and Estonia (0.7). 
...
Next I test the predictive power of my model by calculating the proportion of correctly classified observations. The proportion of inccorrectly classified observations is the error of my model. In the crosstable below we see the confusion matrix comparing the real and the predicted model. 
...
Next I define a loss function and compute the mean prediction error for my model. Below we see that the training error of the model is 0.18. It means that, on average, 18 % of the predictions were wrong. Simple guessing method in a binary situation (e.g. flipping a symmetric coin) would have been right roughly in 50 % of the events. My model predicted accurately in 82 % of situations. Hence, my model performed better than just guessing. Still, interestingly, belongin to religion is complicated question: it seems to be something else than just the amount of praying, attending to religious services, and seeing oneself as religious.
